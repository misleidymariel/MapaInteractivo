direccionesModulo = (function () {
  ...
  function inicializar() {
    ...
    $('#agregar').keypress(function (e) {
      if (e.keyCode == 13) {
        var direccion = addCity(document.getElementById('agregar').value)
  ...
  function calcularYMostrarRutas() {
    ...
    let desde = addCity(document.getElementById("desde").value);
    
    let hasta = addCity(document.getElementById("hasta").value);
    let intermedios = document.getElementById("puntosIntermedios");
    let waypoints = [];
    for (i = 0; i < intermedios.length; i++) {
      
      let item = intermedios.options[i].value;
      let splitItem = item.split(',');
      let latitude = Number(splitItem[0]);
      let longitude = Number(splitItem[1]);
